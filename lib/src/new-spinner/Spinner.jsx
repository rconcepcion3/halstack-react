...
const DxcNewSpinner = ({ label = "", value, showValue = false, mode = "large", margin }) => {
            ...
            <Spinner mode={mode}>
              {mode !== "small" && (
                <SVGSpinner viewBox="0 0 140 140" isDeterminated={true}>
                  <CircleSpinner cx="70" cy="70" r="65" mode={mode} isDeterminated={true} value={value}></CircleSpinner>
                </SVGSpinner>
              )}
              {mode === "small" && (
                <SVGSpinner viewBox="0 0 16 16" isDeterminated={true}>
                  <CircleSpinner cx="8" cy="8" r="6" mode={mode} isDeterminated={true} value={value}></CircleSpinner>
                </SVGSpinner>
              )}
            </Spinner>
          ) : (
            <Spinner mode={mode}>
              {mode !== "small" && (
                <SVGSpinner viewBox="0 0 140 140" isDeterminated={false}>
                  <CircleSpinner cx="70" cy="70" r="65" mode={mode} isDeterminated={false}></CircleSpinner>
                </SVGSpinner>
              )}
              {mode === "small" && (
                <SVGSpinner viewBox="0 0 16 16" isDeterminated={false}>
                  <CircleSpinner cx="8" cy="8" r="6" mode={mode} isDeterminated={false}></CircleSpinner>
                </SVGSpinner>
              )}
            </Spinner>
          )}
          {mode !== "small" && (
            <LabelsContainer label={label} value={value} showValue={showValue}>
              ...
              {(value || value === 0) && showValue && (
                <SpinnerProgress mode={mode} showValue={showValue}>
                  {value}%
                </SpinnerProgress>
              )}
            </LabelsContainer>
...
const CircleSpinner = styled.circle`
  ...
  stroke: ${(props) => (props.mode !== "overlay" ? props.theme.trackCircleColor : props.theme.trackCircleColorOnDark)};
